...
namespace ZatcaService.Helpers
{
    ...
    {
        public static Invoice GenerateInvoiceObject(RelayData relayData, GatewaySetting gatewaySetting, Int32 iCv, string pIh)
        {
            ...
            string invoiceSubType = managerInvoice.CustomFields2.Strings[gatewaySetting.InvoiceSubTypeGuid] ?? "0200000";
            ...
            invoice.AccountingSupplierParty = CreateAccountingSupplierParty(gatewaySetting);
            
            string partyTaxInfoJson = managerInvoice.InvoiceParty.CustomFields2.Strings[gatewaySetting.PartyTaxInfoGuid];
            invoice.AccountingCustomerParty = CreateAccountingCustomerParty(partyTaxInfoJson);

            invoice.Delivery = new Delivery()
            {
                ActualDeliveryDate = managerInvoice.IssueDate.ToString("yyyy-MM-dd"),
                LatestDeliveryDate = managerInvoice.IssueDate.ToString("yyyy-MM-dd")
            };

            invoice.PaymentMeans = CreatePaymentMeans(managerInvoice, gatewaySetting);

            //AllowanceCharge on Document Level
            //invoice.AllowanceCharge = CreateAllowanceCharge(mi, invoiceCurrencyCode, gatewaySetting);
            invoice.InvoiceLine = CreateInvoiceLines(managerInvoice, invoiceCurrencyCode, gatewaySetting).ToArray();
            invoice.TaxTotal = CalculateTaxTotals(managerInvoice, invoiceCurrencyCode, TaxCurrencyCode, gatewaySetting).ToArray();
        ...
        private static AccountingSupplierParty CreateAccountingSupplierParty(GatewaySetting gatewaySetting)
        {
            return new AccountingSupplierParty
            {
                Party = new Party
                {
                    PartyIdentification = new PartyIdentification
                    {
                        ID = new ID
                        {
                            SchemeID = gatewaySetting.SchemeID,
                            Value = gatewaySetting.ID
                        }
                    },
                    PostalAddress = new PostalAddress
                    {
                        StreetName = gatewaySetting.StreetName,
                        BuildingNumber = gatewaySetting.BuildingNumber,
                        CitySubdivisionName = gatewaySetting.CitySubdivisionName,
                        CityName = gatewaySetting.CityName,
                        PostalZone = gatewaySetting.PostalZone,
                        Country = new Country
                        {
                            IdentificationCode = gatewaySetting.CountryIdentificationCode
                        }
                    },
                    PartyTaxScheme = new PartyTaxScheme
                    {
                        CompanyID = gatewaySetting.CompanyID,
                        TaxScheme = new TaxScheme
                        {
                            ID = new ID(gatewaySetting.TaxSchemeID)
                        }
                    },
                    PartyLegalEntity = new PartyLegalEntity
                    {
                        RegistrationName = gatewaySetting.RegistrationName
                    }
                }
            };
        }
        ...
        private static PaymentMeans CreatePaymentMeans(ManagerInvoice managerInvoice, GatewaySetting gatewaySetting)
        {
            ...
            {
                managerInvoice.CustomFields2.Strings.TryGetValue(gatewaySetting.PaymentMeansCodeGuid, out paymentMeans);
                managerInvoice.CustomFields2.Strings.TryGetValue(gatewaySetting.InstructionNoteGuid, out instructionNote);
            }
        ...
        private static AllowanceCharge CreateAllowanceCharge(ManagerInvoice managerInvoice, string currencyCode, GatewaySetting gatewaySetting)
        {
            ...
            if (hasDiscount)
            {
                ...
                foreach (var line in lines)
                {
                    ...
                    string itemTaxCategoryID = line.Item.CustomFields2.Strings[gatewaySetting.ItemTaxCategoryGuid];
        ...
        private static List<InvoiceLine> CreateInvoiceLines(ManagerInvoice managerInvoice, string currencyCode, GatewaySetting gatewaySetting)
        {
            List<Line> lines = managerInvoice.Lines;
        ...
        private static List<TaxTotal> CalculateTaxTotals(ManagerInvoice managerInvoice, string currencyCode, string taxCurrencyCode, GatewaySetting gatewaySetting)
        {
            List<Line> lines = managerInvoice.Lines;
            bool amountsIncludeTax = managerInvoice.AmountsIncludeTax;
